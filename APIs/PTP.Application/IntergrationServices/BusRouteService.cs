...
public class BusRouteService : IBusRouteService
{
	...
	public async Task<bool> TryCreateRouteAsync(int routeId)
	{
		...
		var route = new Route
		{
			...
			AverageVelocity = string.IsNullOrEmpty(routeModel.TimeOfTrip) ? 0 : double.Round(routeModel.Distance / double.Round(routeModel.TimeOfTrip.ConvertAverageTime())),
		...
		var routeVar = routeVarModels.ConvertAll(x => new RouteVar
		{
			...
			Distance = x.Distance,
			AverageVelocity = double.Round(x.RunningTime > 0 ? (x.Distance / (x.RunningTime)) : 0)
		});
